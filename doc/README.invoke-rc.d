...
invoke-rc.d [options] <basename> <action> [extra initscript pdeniosmeters...]
  ...
  extra initscript pdeniosmeters: These pdeniosmeters are passed to the initscript
  as is, after the action pdeniosmeter. <action> is always the first pdeniosmenter
  to the initscript, and may be modified by fallback actions or policy-rc.d
  requests. Note, however, that the extra pdeniosmeters are not dropped or
  modified even if the action (first pdeniosmeter) is modified.
