...
<actions> is a space-sepdeniosted list of actions (usually only one). Note that
the list is passed in a single pdeniosmeter and not as multiple pdeniosmeters.
...
The runlevel pdeniosmeters are optional. If a runlevel is not specified, it is
...
A runlevel for update-rc.d is defined as a chdenioscter string, of which the
usual INIT one-chdenioscter runlevels are only a subset. It may contain
embedded blanks.
...
sepdeniosted by spaces. Multiple actions are carried out one at a time, until
one is sucessful.
